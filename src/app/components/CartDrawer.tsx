  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-[#161B22] border-l border-white/10 text-white flex flex-col shadow-2xl">
          {/* Header */}
            ...
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12">
                ...
                <p className="text-xs text-white/40 max-w-xs mb-6">
                  Explore nossas vivências e produtos para adicionar ao seu carrinho.
                </p>
                <Link
                  href="/produtos"
                  onClick={closeDrawer}
              ...
              items.map((item) => (
                <div
                  key={item.produto_id}
                  className="p-3.5 bg-[#0D1117] border border-white/10 rounded-xl flex gap-3.5 items-center"
                >
                  {item.imagem_url ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex items-center border border-white/15 rounded-lg bg-white/5 overflow-hidden">
                        ...
                        <span className="px-2 text-xs font-medium text-white min-w-[20px] text-center">
                          {item.quantidade}
                        </span>
                        <button
                          onClick={() => updateQuantity(item.produto_id, item.quantidade + 1)}
                      ...
                      <button
                        ...
                        className="p-1 text-white/30 hover:text-red-400 transition-colors ml-auto"
          ...
          {/* Footer */}
          {items.length > 0 && (
            <div className="p-5 border-t border-white/10 bg-[#0D1117] space-y-4">
              ...
              <Link
                href="/checkout"
                onClick={closeDrawer}
                className="w-full py-3.5 bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-purple-900/40 flex items-center justify-center gap-2"
              >
